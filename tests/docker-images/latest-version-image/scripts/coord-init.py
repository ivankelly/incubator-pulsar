...
def usage():
    print >> sys.stderr, "\n%s -z <zookeeper> -p <path> [-w|-c]" % (sys.argv[0])
...
try:
    opts, args = getopt.getopt(sys.argv[1:], "z:p:czh")
...
if (not watch and not create) or (watch and create):
    print >> sys.stderr, "Exactly one of watch (-w) or create (-c) must be specified"
    usage()
    sys.exit(5)
